...
const ROLE_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  PARTNER: { bg: 'bg-blue-50 text-blue-700 border-blue-100', text: 'text-blue-700', label: 'Partner' },
  EMPLOYEE: { bg: 'bg-teal-50 text-teal-700 border-teal-100', text: 'text-teal-700', label: 'Employee' },
  AUDITOR: { bg: 'bg-purple-50 text-purple-700 border-purple-100', text: 'text-purple-700', label: 'Auditor' },
  ACCOUNTANT: { bg: 'bg-amber-50 text-amber-700 border-amber-100', text: 'text-amber-700', label: 'Accountant' },
  TAX_CONSULTANT: { bg: 'bg-rose-50 text-rose-700 border-rose-100', text: 'text-rose-700', label: 'Tax Consultant' },
};
      ...
      render: (member) => {
        ...
        return (
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[12px] font-medium ${active ? 'text-emerald-700' : 'text-slate-500'}`}>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#091124]">Manage Staff</h2>
          <p className="text-slate-500 mt-0.5 text-[13px]">Showing {filteredStaff.length} staff members</p>
        </div>
        <Button 
      ...
      {isLoading ? (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-12 flex flex-col items-center justify-center">
          <div className="w-8 h-8 border-4 border-[#00C2B3] border-t-transparent rounded-full animate-spin animate-duration-500"></div>
          <p className="mt-4 text-slate-500">Loading staff members...</p>
        </div>
      ) : isError ? (
        <div className="bg-red-50 border border-red-100 text-red-600 rounded-2xl p-6 text-center font-medium">
...
export default function StaffPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-slate-500 animate-pulse">Loading Staff Management...</div>}>
      <StaffPageContent />
    </Suspense>
  );
}
