        ...
        {isProcessing ? (
          <div className="py-8 px-4 text-center space-y-6">
            ...
            <div className="flex items-center justify-center gap-2 pt-2 text-[11px] font-medium">
              <span className={`px-2.5 py-1 rounded-full ${processingStage === 'upload' ? 'bg-emerald-600 text-white font-bold' : 'bg-slate-500/10 text-slate-500'}`}>
                1. Upload
              </span>
              <span className="text-slate-300">→</span>
              <span className={`px-2.5 py-1 rounded-full ${processingStage === 'ocr' ? 'bg-emerald-600 text-white font-bold' : 'bg-slate-500/10 text-slate-500'}`}>
                2. AI OCR
              </span>
              <span className="text-slate-300">→</span>
              <span className={`px-2.5 py-1 rounded-full ${processingStage === 'mapping' ? 'bg-emerald-600 text-white font-bold' : 'bg-slate-500/10 text-slate-500'}`}>
                3. Ledger Mapping
              </span>
            </div>
          </div>
          ...
          <form onSubmit={handleUpload} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold flex items-center gap-1.5" style={{ color: 'var(--color-text-secondary)' }}>
                <Building2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Select Client Entity <span className="text-rose-500">*</span>
              </label>
              <select
                value={clientId}
                onChange={(e) => setClientId(e.target.value)}
                required
                className="w-full text-xs px-3 py-2.5 rounded-xl border bg-[var(--color-bg-card)] border-[var(--color-border)] text-[var(--color-text-primary)] focus:outline-none focus:border-emerald-500 cursor-pointer"
              >
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all ${
                  isDragOver
                    ...
                    : 'border-[var(--color-border)] hover:border-emerald-500/60'
                }`}
              >
                <input
                  type="file"
